...
function Details() {
  ...
  const averageRatings = useMemo(() => {
    if (gigData && gigData.reviews.length) {
      const totalRating = gigData.reviews.reduce((sum, { rating }) => sum + rating, 0);
      return (totalRating / gigData.reviews.length).toFixed(1);
    }
    return "0";
  }, [gigData]);
  ...
  if (error) {
    return <div>Error loading gig data: {error}</div>; // Display error message
  }
  ...
  return (
    <>
      {gigData && currentImage && (
        <div className="col-span-2 flex flex-col gap-3">
          <h2 className="text-2xl font-bold text-[#404145] mb-1">{gigData.title}</h2>
          <div className="flex items-center gap-2">
            <div>
              {gigData.createdBy.profileImage ? (
                <Image
                  src={`${HOST}/${gigData.createdBy.profileImage}`}
                  alt={`${gigData.createdBy.fullName}'s profile`}
                  height={30}
                  width={30}
                  className="rounded-full"
                />
                ...
                <div className="bg-purple-500 h-10 w-10 flex items-center justify-center rounded-full">
                  <span className="text-xl text-white">{gigData.createdBy.email[0].toUpperCase()}</span>
                </div>
              )}
            </div>
            <div className="flex gap-2 items-center">
              ...
              {gigData.images.length > 1 &&
                gigData.images.map((image) => (
                  <Image
                    ...
                    key={image}
                    onClick={() => setCurrentImage(image)}
                    className={`cursor-pointer transition-all duration-500 ${currentImage === image ? "" : "blur-sm"}`}
                  />
                ))}
            </div>
          </div>
          <div>
            <h3 className="text-3xl my-5 font-medium text-[#404145]">About this gig</h3>
            <p>{gigData.description}</p>
          </div>
          {/* About the seller */}
          <div>
            <h3 className="text-3xl my-5 font-medium text-[#404145]">About the Seller</h3>
            <div className="flex gap-4">
              <div>
                {gigData.createdBy.profileImage ? (
                  <Image
